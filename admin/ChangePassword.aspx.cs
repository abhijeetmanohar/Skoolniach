...
public partial class admin_ChangePassword : System.Web.UI.Page
{
    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        {
            c.Open();
            SqlCommand cmd = new SqlCommand("select * from Login where UserId='" + txtUI.Text + "'", c);
            if (txtNP.Text == txtCNP.Text)
            {
                ...
                if (dr.Read())
                {
                    if (dr["Password"].ToString() == txtOP.Text)
                    {
                        c.Close();
                        c.Open();
                        SqlCommand cmd1 = new SqlCommand("Update Login set Password='" + txtNP.Text + "' where UserId='" + txtUI.Text + "' and Category='admin'", c);
